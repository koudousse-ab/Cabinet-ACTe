...
function getClassListFromProps(props) {
  ...
  if (canvasSquare)
    result.push(STYLE_CLASSES.canvasSquare);
  if (canvasRoomy)
    result.push(STYLE_CLASSES.canvasRoomy);
  if (flip360)
    result.push(ANIMATION_CLASSES.flip360);
  if (buzz)
    result.push(ANIMATION_CLASSES.buzz);
  if (float)
    result.push(ANIMATION_CLASSES.float);
  if (jello)
    result.push(ANIMATION_CLASSES.jello);
  if (spinSnap)
    result.push(ANIMATION_CLASSES.spinSnap);
  if (spinSnap4)
    result.push(ANIMATION_CLASSES.spinSnap4);
  if (spinSnap8)
    result.push(ANIMATION_CLASSES.spinSnap8);
  if (swing)
    result.push(ANIMATION_CLASSES.swing);
  if (wag)
    result.push(ANIMATION_CLASSES.wag);
  const prefix = config$1.cssPrefix || config$1.familyPrefix || DEFAULT_CLASSNAME_PREFIX;
