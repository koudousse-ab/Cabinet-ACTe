...
import './EnseignantStats.css';

export default function ErrorCounter({ enseignantId, errors, onErrorAdded }) {
 ...
 setSubmitting(true);
 enseignantApi
 .addError({ enseignantId, description: description.trim(), date: new Date().toISOString().slice(0, 10) })
 .then(() => {
 setDescription('');
 onErrorAdded();
 })
