...
export default function EnseignantForm({ enseignant, onSave, onClose }) {
 const [formData, setFormData] = useState(EMPTY_FORM);
 const isEditing = !!enseignant;

 useEffect(() => {
 if (enseignant) {
 setFormData({ ...EMPTY_FORM, ...enseignant, password: '' });
 } else {
 ...
 alert('Le mot de passe est obligatoire pour un nouvel enseignant');
 return;
 }
 ...
 <h3>{isEditing ? 'Éditer un enseignant' : 'Ajouter un enseignant'}</h3>
